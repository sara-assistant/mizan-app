...
export default function CasesPage() {
  ...
  const getStatusLabel = (status: string) => {
    if (status === "active") return t("cases.active").toUpperCase();
    if (status === "pending") return t("cases.pending").toUpperCase();
    if (status === "won") return t("cases.won").toUpperCase();
    if (status === "lost") return t("cases.lost").toUpperCase();
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="page-header">
        <div className="page-title">{t("nav.cases").toUpperCase().split(" ").slice(0, 1).join(" ")}</div>
        <span className="page-title-grad">{t("cases.title").toUpperCase()} ›</span>
        <Link href="/submit" className="flex items-center gap-2 bg-[#1C1C1E] rounded-xl px-4 py-2.5 w-fit">
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
            <path d="M7 1v12M1 7h12" stroke="white" strokeWidth="2" strokeLinecap="round" />
          </svg>
          <span className="text-[13px] font-black text-white">{t("cases.add_case")}</span>
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 flex flex-col gap-3">
        {mockCases.length === 0 ? (
          <div className="text-center py-12">
            <div className="text-[15px] text-[#8E8E93]">{t("cases.no_cases")}</div>
          </div>
        ) : (
          mockCases.map((c) => (
            <Link href="/case-passport" key={c.id} className="bg-white rounded-2xl overflow-hidden shadow-sm active:scale-[0.98] transition-transform">
              <div className="p-4">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <Tag variant={statusVariant(c.status)}>{getStatusLabel(c.status)}</Tag>
                </div>
              ...
              {c.actionDue && (
                <div className="border-t border-[#F2F2F7] px-4 py-2.5 flex items-center justify-between">
                  ...
                  <span className="text-[11px] font-bold text-[#FF6B00]">{t("common.learn_more")} →</span>
                </div>
              )}
            </Link>
          ))
